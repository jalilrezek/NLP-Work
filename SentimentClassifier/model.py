...
def featurize(sentence: str, embeddings: gensim.models.keyedvectors.KeyedVectors) -> Union[None, torch.FloatTensor]:
    ...
    if len(vectors) == 0: # vector sequence is empty (happens if sentence is empty, or if none of the words in the sentence is in embed vocab)
        return None

    avg = np.zeros_like(vectors[0]) # initialize a numpy array of 0s whose length is the length of vectors[0] i.e. the embedding vectors.
    for vector in vectors: # note: could use np.mean(vectors, axis=0) for simplicity. Did it manually to learn better.
        avg = avg + vector
    avg = avg / len(vectors)
...
def create_tensor_dataset(raw_data: Dict[str, List[Union[int, str]]],
    ...
    for text, label in tqdm(zip(raw_data['text'], raw_data['label'])):
        ...
        # featurize each sentence. Only add feature & label to the list if the feature is not None
        averages = featurize(text, embeddings) # averages vector for current movie review
        if (averages is not None):
            all_features.append(averages)
            all_labels.append(torch.tensor(label, dtype = torch.long)) # only add label if the featuer is not none.
            # the feature ("averages") is already a tensor so no need to convert to tensor.
            # but the label might be just an int so do need to convert to tensor.
...
class SentimentClassifier(nn.Module):
    def __init__(self, embed_dim, num_classes):
        ...
        self.projection = nn.Linear(embed_dim, num_classes) # project from embedding_dim to num_classes using nn.Linear (linear matrix transformation)
...
def accuracy(logits: torch.FloatTensor, labels: torch.LongTensor) -> torch.FloatTensor:
    ...
    # complete the function to compute the accuracy. Will return a tensor of 0s and 1s with the same shape as labels.

    # labels is a tensor of shape (batch_size,)
    # logits is a tensor of shape (batch_size, num_classes)
    predictions = torch.argmax(logits, dim = 1) # dim = 1 means to go over the "column" dimension for each row. So this will give an array/vector
    # where each index is the max of that row of the "logits" tensor. 
    res = (predictions==labels).float() # if predictions = (1,1,0) and labels = (0,1,0) then res = (False,True,True).float() = (0,1,1)
